...
def main(fnames):
    ...
    map_algos_colors = {'trpo': True, 'qpropconserv': True, 'qpropconserveta': True}
    ...
    for i, fname in enumerate(fnames):
        ...
        if map_algos_colors[algo] == True:
            plt.plot(eps, savgol_filter(rews, 101, 5), color=descs_d[algo], label="{}".format(algo), alpha=0.4)
            map_algos_colors[algo] = False
        else:
            plt.plot(eps, savgol_filter(rews, 101, 5), color=descs_d[algo], alpha=0.4)
